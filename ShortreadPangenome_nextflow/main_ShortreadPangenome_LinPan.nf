...
//then finally output short-read-based linear pangenome
...
process step2_4{
    ...
    script:
    """
    bash Step2.4_FromAssemblingToLinPan_nf.sh ${sra} ${ref} ${linpan} ${contig} ${GENOME} ${N} ${APP} ${MASURCA} ${GENOME_SIZE} ${PREFIX}
    """
}
...
process step2_6{
    tag "$step2_6"
    clusterOptions '--job-name=Step2.6 -n 64 -N 1 -t 1-00:00:00 -A fnrdewoody --mail-user $params.email --mail-type END,FAIL'
    //errorStrategy 'ignore' #don't want to ignore errors for this process
...
workflow{
    ...
    Channel.fromPath(params.csv) \
        ...
        | map { row-> [row.sra, row.db, row.ref, row.linpan, row.contig] } \
        | view() \
        | step1 \
        | step2_1 \
        | step2_2 \
