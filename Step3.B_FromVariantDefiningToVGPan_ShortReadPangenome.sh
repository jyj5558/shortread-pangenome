...
# Augmenting the graph pangenome (preprocessing steps before alignment were modified from Sacomandi et al. (2023)'s scripts)
cd ${VGPAN}/
...
${APP}/vg augment -t ${N} ${PREFIX}2.pg combined_aln.gam -s -m 3 -q 5 -Q 5 -A ${PREFIX}2_aug.gam > ${PREFIX}2_aug.pg #-s: safely ignore alignments to nodes outside the graph; -m 3: minimum coverage of 3; -q & -Q 5: filtering out mappings and bases with quality < 5 
