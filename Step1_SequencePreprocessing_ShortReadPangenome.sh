...
for g in `ls -lt ../cleaned | grep "fq.gz" | tr -s ' ' | cut -d " " -f 9 | cut -d "R" -f 1 | uniq`
  do
  kraken2 --db ${DB} --quick --threads ${KRAKEN2_NUM_THREADS} --output ${g}cseq --paired --unclassified-out ${g}filtered#.fq ${SRA}/cleaned/${g}R1_001_val_1.fq.gz ${SRA}/cleaned/${g}R2_001_val_2.fq.gz --report ${g}kraken_report.txt --use-names
done
cd ../
